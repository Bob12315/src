...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gimbal-controller",
        action="store_true",
        help="Enable only selected controllers; if none of the controller flags are set, all controllers are enabled",
    )
    parser.add_argument(
        "--body-controller",
        action="store_true",
...
def _resolve_controller_selection(args: argparse.Namespace) -> dict[str, bool]:
    selected_any = any(
        (
            bool(args.gimbal_controller),
            bool(args.body_controller),
            bool(args.approach_controller),
        )
    )
    if not selected_any:
        return {
    ...
    return {
        "gimbal": bool(args.gimbal_controller),
        "body": bool(args.body_controller),
        "approach": bool(args.approach_controller),
    }
...
def main() -> int:
    ...
    controller_enabled = _resolve_controller_selection(args)
    ...
    logger.info(
        "starting control service yolo_udp=%s:%s loop_hz=%.2f telemetry_source=%s controllers=gimbal:%s body:%s approach:%s",
        yolo_udp_ip,
        yolo_udp_port,
        loop_hz,
        telemetry_cfg.active_source,
        controller_enabled["gimbal"],
        controller_enabled["body"],
        controller_enabled["approach"],
    )
